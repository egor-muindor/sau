...
1. Если `partIndex < len(serverUrls)`: хост `serverUrls[partIndex]`, если он не помечен
   сбойным; иначе — случайный не сбойный. Замеры скорости на этом шаге **не** используются.
2. Если `partIndex >= len(serverUrls)` и замеры есть — самый быстрый по замерам
   (`размер чанка / длительность`), не сбойный.
3. Иначе — случайный не сбойный.
4. Если сбойные все — сбросить отметки и взять случайный.

(В браузере на шаге 1 перед случайным выбором ещё проверяется «самый быстрый из
прошлого сеанса» из `localStorage`; для CLI аналога нет, шаг опущен.)
